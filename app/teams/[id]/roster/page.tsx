...
export default function TeamRosterPage() {
  ...
  async function load() {
    ...
    // 1. get current term id
    const termId = localStorage.getItem('kauri.termId');
    if (!termId) {
      setMsg('No term selected. Please select a term from the Terms page.');
      setLoading(false);
      return;
    }
    ...
    if (teamError || !teamData) {
      setMsg('Team not found or error loading team.');
      setLoading(false);
      return;
    }
    setTeam(teamData as Team);

    // 3. load memberships for this term + team
    ...
    // Shape data
    const shaped: RosterRow[] = data.map((m: any) => {
      const pl = m.players;
      return {
        membershipId: m.id,
        playerId: pl.id,
        ...
        preferredName: pl.preferred_name,
        jerseyNo: pl.jersey_no,
        role: m.role || 'player',
      };
    });
