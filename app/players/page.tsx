...
type Player = {
  ...
  preferred_name: string | null; jersey_no: number | null;
  status: string | null;
};
    ...
    <main className="min-h-screen bg-neutral-50 text-neutral-900 p-6">
      <div className="max-w-5xl mx-auto space-y-4">
        <header className="flex items-center justify-between">
          ...
          <a href="/players/new" className="px-3 py-2 rounded-md bg-blue-700 hover:bg-blue-800 text-white font-semibold">Add player</a>
        </header>
        ...
        <section className="bg-white border border-neutral-200 rounded-xl shadow-sm">
          <table className="w-full text-sm">
            <thead className="border-b border-neutral-200 bg-neutral-100 text-neutral-900">
              <tr>
            ...
            <tbody>
              {filtered.map(p => (
                <tr key={p.id} className="border-b border-neutral-100 hover:bg-neutral-50">
                  <td className="p-3">
                    <a href={`/players/${p.id}`} className="underline text-blue-700 hover:text-blue-800">
                  ...
                  <td className="p-3">
                    <a href={`/players/${p.id}/edit`} className="underline text-blue-700 hover:text-blue-800">Edit</a>
                  </td>
                </tr>
              ))}
              {filtered.length === 0 && (
